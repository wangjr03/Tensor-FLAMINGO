...
_`input_folder:`_  The folder path to input data, can only contain scHi-C data files.  
_`chr_name:`_  The desired chromosome, e.g."chr19"  
_`low_res:`_  The domain-level low resolution used for FLAMINGO reconstruction.  
_`high_res:`_  The bin-level high resolution desired for final results.  
_`assembly:`_  The genome assembly version of input data.  
_`outputs_folder:`_  The folder to store all intermediate and final outputs.  
_`code_path:`_  The path of where tFLAMINGOrLite is, i.e."../Tensor-FLAMINGO"  
