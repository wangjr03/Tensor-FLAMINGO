...
#' @param temp_folder temporary folder to store domain inputs
...
#' @export
flamingo.divide_domain <- function(temp_folder,flamingo_obj,domain_res,frag_res){
  options(scipen = 999)
  ...
  for(i in 1:n_domain){
    ...
    write.table(as.matrix(tmp_pd),paste0(temp_folder,"/Domain_data/PD_domain_",i,'.txt'),col.names = F,row.names = F,sep="\t",quote=F)
    write.table(as.matrix(tmp_input_if),paste0(temp_folder,"/Domain_data/IF_domain_",i,'.txt'),col.names = F,row.names = F,sep="\t",quote=F)
