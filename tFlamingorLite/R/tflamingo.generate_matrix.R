...
tflamingo.generate_matrix <- function(input_path,resolution,chr_name,opt_path,assembly){
    setwd(input_path)
    file_list=dir(input_path)
    print("Runing generate matrix")
    suffix=1
    for(fn in file_list){
        tflamingo.process_one_file(fn,resolution,chr_name,opt_path,assembly,suffix)
        suffix = suffix+1
    }
}
