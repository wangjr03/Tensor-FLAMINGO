...
#' @export
flamingo.reconstruct_structure<- function(temp_folder,sw,lambda,max_dist,nThread){
  require(parallel)
  setwd(temp_folder)

  cl <- parallel::makeCluster(nThread)
  parallel::clusterCall(cl, function() library(tFlamingorLite))

  file <- dir(paste0(temp_folder,'/Domain_data/'))
  n <- length(grep('IF',file))
  res = list()
  domain_id <- c()
