                            ...
                            nThread)
{
  ...
  res <- parallel::parSapply(cl,domain_id,function(index){
    # read data
    tmp_input_pd <- read.table(paste0(temp_folder,"/domain_data/PD_domain_",index,".txt"))
    tmp_res =  flamingo_basic(tmp_input_pd,sample_rate,lambda,r,max_dist,error_threshold,max_iter,alpha,inf_dist)
    return(tmp_res)
